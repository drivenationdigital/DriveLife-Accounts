/**
 * Official payment-provider logos, inlined as SVG so they render with no
 * network fetch and inherit sizing from a className. All are decorative
 * (aria-hidden) - the card headings already name each provider.
 *
 * Sources: Stripe/PayPal/Mollie wordmarks and the Square mark are the
 * providers' own vector logos (via worldvectorlogo / simple-icons),
 * untouched apart from JSX attribute casing and the Stripe fill being
 * updated to the current brand purple.
 */
...
export function MollieLogo({ className }: { className?: string }) {
  return (
    <svg
